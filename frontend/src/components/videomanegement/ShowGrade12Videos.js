...
export default function ShowGrade13Videos() {
  ...
  const [allGrade12, setGrade12] = useState([]);

  useEffect(() => {
    function getVideo() {
      axios.get('http://localhost:3000/video/')
        .then((res) => {
          var videoGrade13 = res.data.filter((video) => video.grade === 12);
          setGrade12(videoGrade13);
        })
        .catch((error) => {
          console.log(error);
          alert('Error');
  ...
  function updatedViews(id, item) {
    const views = item + 1;
    const updatedGrade13 = allGrade12.map((video) => {
      if (video._id === id) {
        return { ...video, views };
      }
      return video;
    });
    setGrade12(updatedGrade13);
    viewI(id, views);
  }
  ...
  return (
    <div class="p-[30px] bg-gradient-to-r from-[#000000] bg-red-800">
      <div>
         <h1 class="text-white p-2 text-[20px] font-bold">Video Lesson Grade 12</h1>
      </div>
      <table className="table table-white text-left bg-gradient-to-r from-[#000000] bg-red-800">
       
        <tbody>
          {allGrade12.map((video) => (
            <tr key={video._id}>
              <td>
